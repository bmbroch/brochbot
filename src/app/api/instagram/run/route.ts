...
export async function POST(req: NextRequest) {
  ...
  try {
    if (firstFetch) {
      ...
      // Register webhooks via Apify Webhooks API
      if (webhookUrl) {
        await Promise.all([
          registerWebhook(runId, webhookUrl),
          registerWebhook(profileRunId, webhookUrl),
        ]);
      }
      ...
      // Register webhook via Apify Webhooks API
